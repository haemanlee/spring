...
### 2-1. 주문 생성
1. 요청의 상품 ID 목록으로 상품 카탈로그 조회
2. 각 상품의 현재 이름/가격을 `OrderItem` 스냅샷 필드로 복사
3. `Order` 루트에 `OrderItem`을 연결 후 저장
4. 응답에는 주문번호, 상태(`CREATED`), 총액 포함

핵심 포인트:
- "상품 마스터 값 참조"가 아니라 "주문 시점 값 복사"가 본질
- 금액 계산 기준은 항상 스냅샷

### 2-2. 상태 변경
1. `findByIdForUpdate` 등으로 대상 주문 잠금 조회
2. 현재 상태와 요청 상태 비교
3. 같으면 no-op(`false` 또는 200 + unchanged)
4. 다르면 상태 변경 + 이력 INSERT

핵심 포인트:
- 이력은 UPDATE/DELETE 없이 누적
- 상태 전이 검증(예: `CREATED -> SHIPPED` 금지)은 서비스 계층 규칙으로 명시

### 2-3. 조회 최적화
- 주문 목록 화면: `OrderSummary` DTO 전용 쿼리
- 상세 화면: 주문 헤더 + 아이템 조회 전략 분리
- 초기 단계는 `fetch join`으로 시작하되, 페이징 시 컬렉션 fetch join 남용 금지
